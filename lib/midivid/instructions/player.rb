module MIDIVid
  ...
  module Instructions
    ...
    module Player
      ...
      # Add all of the player methods to the context as instructions
      def method_missing(method, *args, &block)
        if @player.player_respond_to?(method)
          @player.player_send(method, *args, &block)
        else
          super
        end
      end
      ...
      def respond_to_missing?(method, include_private = false)
        super || @player.player_respond_to?(method)
      end
